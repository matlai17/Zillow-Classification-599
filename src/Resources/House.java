...
public class House {
	...
	public House(long zid, String street, String city, String state,
			String zip, double priceSold, int bedrooms, double bathrooms,
			double area, int year, double zestimate, int elem, int mid, int high) {
	 ...
	 * Can be iterated through to retrieve both the key and the value, i.e. the
	 * feature name and the value.
	 * 
	...
	/**
	 * getFeaturesArray returns an array rather than a map of the features. The
	 * final element in the array contains the price. Might be easier to use
	 * than the getFeatures method, which returns only a map of the features and
	 * requires a separate call to retrieve the sold price.
	 * 
	 * @return A double array containing the features.
	 */
	public double[] getFeaturesArray() {
		return new double[] { Double.parseDouble(houseAddress.getZip()),
				noOfBedroom, noOfBathroom, area, builtYear, schoolElem,
				schoolMid, schoolHigh, priceSold };
	}

	// Returns the House Address
	public String getHouseCompleteAddress() {
		return new String(houseAddress.getstreetAddress() + " "
				+ houseAddress.getCity() + " " + houseAddress.getState());
	}
	...
	public java.util.List<String[]> getInfo() {
		...
		info.add(new String[] { "Street Address",
				houseAddress.getstreetAddress() });
	...
	public void setHouseAddress(String streetAddress, String city,
			String state, String zipCode) {
		this.houseAddress = new HouseAddress(streetAddress, city, state,
				zipCode);
	}
